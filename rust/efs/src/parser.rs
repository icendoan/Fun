    ...
    match mode {
        ...
        LexMode::Num => {
            if peek.is_digit(10) || peek == '.' {
                ...
                match (parse_int(src), parse_float(src), parse_date(src)) {
                    ...
                    (None, None, Some((y, m, d))) => Ok((Token::Date(y, m, d), LexMode::Unknown)),
                    (None, None, None) => {
        ...
        LexMode::String => {
            ...
            if src.len() > 1 && src[src.len() - 1] == '"' {
                Ok((Token::Str(to_string(&src[1..src.len() - 1])),
                    LexMode::Unknown))
            } else {
                Ok((Token::Placeholder, mode))
            }
        ...
        LexMode::Unknown => {
            if src.len() == 0 {
                match peek {
                    ...
                    '=' => Ok((Token::Placeholder, mode)),
                    '-' => Ok((Token::Placeholder, mode)),
                    c if c.is_whitespace() => Ok((Token::Placeholder, mode)),
                    '"' => Ok((Token::Placeholder, LexMode::String)),
                    c if c.is_digit(10) => Ok((Token::Placeholder, LexMode::Num)),
                ...
                match src {
                    ...
                    [c] if c.is_whitespace() => Ok((Token::Delim, next_mode)),
                    ['"'] => Ok((Token::Placeholder, LexMode::String)),
                    [c] |
                    ['-', c] if c.is_digit(10) => Ok((Token::Placeholder, next_mode)),
                    _ => Ok((Token::Placeholder, LexMode::Ident)),
...
fn to_regtype(s: &str) -> Option<Regtype> {
    if s == "irregular" {
        return Some(Regtype::Irr);
    }
    if s == "cavalry" {
        return Some(Regtype::Cav);
    }
    if s == "infantry" {
        return Some(Regtype::Inf);
    }
    if s == "hussar" {
        return Some(Regtype::Hus);
    }
    if s == "cuirassier" {
        return Some(Regtype::Cui);
    }
    if s == "dragoon" {
        return Some(Regtype::Dra);
    }
    if s == "artillery" {
        return Some(Regtype::Art);
    }
    if s == "engineer" {
        return Some(Regtype::Eng);
    }
    if s == "guard" {
        return Some(Regtype::Gua);
    }
    if s == "tank" {
        return Some(Regtype::Arm);
    }
    if s == "plane" {
        return Some(Regtype::Pla);
    }
...
pub fn parse<'a>(path: &Path) -> PResult<Save> {
    ...
    let save = try!(root(&mut tokens));

    Ok(save)
}

fn root(tokens: &mut Scanner) -> PResult<Save> {
    let mut nations: Save = BTreeMap::new();
    let mut provinces: BTreeMap<i64, Province> = BTreeMap::new();
        ...
        match tokens.next() {
            Some(Token::Ident(s)) => {
                if is_tag(&s) {
                    ...
                    let n = try!(nation(tokens, &mut tag));
                    if let Some(_) = nations.insert(tag.clone(), n) {
                        return Err(PError::new("Unexpected duplicate nation", None, None));
                    }
                }
            }
            ...
            Some(Token::Int(i)) => {
                match tokens.next() {
                    Some(Token::Eq) => {
                        let p = try!(province(tokens));
                        if let Some(_) = provinces.insert(i, p) {
                            return Err(PError::new("Unexpected duplicate province", None, None));
                        }
                    }
                    _ => {}
                }
...
fn nation(tokens: &mut Scanner, tag: &mut Tag) -> PResult<Nation> {
    match tokens.next() {
        ...
        t => return Err(PError::new("Not a tag", t, Some(Token::Ident("TAG".to_string())))),
    }
    ...
    loop {
        match tokens.next() {
            Some(Token::Ident(s)) => {
                match &s as &str {
                    "army" => {
                        tokens.put_back(Token::Ident(s.clone()));
                        try!(army(tokens, tag, &mut nation.armies));
                    }
                    "navy" => {
                        tokens.put_back(Token::Ident(s.clone()));
                        try!(navy(tokens, tag, &mut nation.navies));
                    }
                    "treasury" => {
                        tokens.put_back(Token::Ident(s.clone()));
                    ...
                    ref t if is_tech(t) => {
                        tokens.put_back(Token::Ident(s.clone()));
                        try!(tech(tokens, &mut nation.tech));
                    }
                    _ => (),
                }
            }
            ...
            None if num_braces > 1 => return ueof::<Nation>(Token::CloseBrace),
            None if num_braces == 1 => return Ok(nation),
...
fn army(tokens: &mut Scanner, tag: &mut Tag, res: &mut Vec<Army>) -> PResult<()> {
    ...
    match tokens.next() {
        ...
        t => return Err(PError::new("Not an army", t, Some(Token::Ident("army".to_string())))),
    }
            ...
            match tokens.next() {
                Some(Token::Ident(s)) => {
                    if let Some(regtype) = to_regtype(&s) {
                        ...
                        // tokens.put_back(Token::Ident(s));
                        Err(PError::new("Not regiment type", None, None))
                    }
                }
...
fn navy(tokens: &mut Scanner, tag: &mut Tag, res: &mut Vec<Navy>) -> PResult<()> {
    let mut navy: Navy = Navy::new();
    let mut num_braces: u64 = 0;
    ...
    match tokens.next() {
        ...
        t => return Err(PError::new("Not a navy", t, Some(Token::Ident("navy".to_string())))),
    }
...
fn ship(tokens: &mut Scanner, tag: &mut Tag, res: &mut Vec<Navy>, navy: &mut Navy) -> PResult<()> {
    let mut num_braces: u64 = 0;
    // println!("Found ship...");
            ...
            match tokens.next() {
                Some(Token::Ident(s)) => {
                    if &s == "clipper_transport" {
                        ...
                        navy.fri += 1;
                        Ok(())
                    } else if &s == "commerce_raider" {
                        navy.com += 1;
                        Ok(())
                    } else if &s == "steam_transport" {
                        navy.tra += 1;
                        Ok(())
                    } else if &s == "monitor" {
                        navy.mon += 1;
                        ...
                        navy.bat += 1;
                        Ok(())
                    } else if &s == "dreadnought" {
                        navy.dre += 1;
                        Ok(())
                    } else {
                        Err(PError::new("Not a shiptype", None, None))
                    }
                }
...
fn treasury(tokens: &mut Scanner, tag: &Tag, money: &mut f64) -> PResult<()> {
    match tokens.next() {
        Some(Token::Ident(s)) => {
            match &s as &str {
                "treasury" => {
                    match tokens.next() {
                        Some(Token::Eq) => {
                            match tokens.next() {
                                ...
                                Some(t) => {
                                    Err(PError::new("Unexpected token",
                                                    Some(t),
                                                    Some(Token::Float(0.0))))
                                }
                                None => ueof(Token::Float(0.0)),
                            }
                        }

                        Some(t) => Err(PError::new("Unexpected token", Some(t), Some(Token::Eq))),
                        _ => ueof(Token::Eq),
                    }
                }
                _ => {
                    Err(PError::new("Bad ident",
                                    Some(Token::Ident(s)),
...
fn primary(tokens: &mut Scanner, tag: &Tag, accepted: &mut Vec<String>) -> PResult<()> {
    Ok(())
}
...
fn is_tech(t: &str) -> bool {
    match t {
        "medicine" => true,
        "breech_loaded_rifles" => true,
        _ => false,
        // match for string literals here
    }
}

fn tech(tokens: &mut Scanner, tech: &mut Tech) -> PResult<()> {
    Ok(())
}

fn province(tokens: &mut Scanner) -> PResult<Province> {
    Ok(Province::new())
}
...
#[inline]
fn ueof<T>(expected: Token) -> PResult<T> {
