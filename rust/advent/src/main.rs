...
fn test_day4()
{
   ...
   caesar_shift("qzmt-zixmtkozy-ivhz".as_bytes(), 343, &mut decryped_name);
              ...
              -> u32
{
   ...
   let room_number = if let Ok(x) = u32::from_str_radix(&room_spec[name_end_idx + 1..code_idx], 10)
   {
      x
   }
   else
...
fn day6()
{
   ...
   println!("Message: {}; Min Message: {}", message, min_msg);
}
...
fn day7()
{
   ...
   println!("TLS addresses: {}, SSL addresses: {}", tls_count, ssl_count)
}
...
fn day7_test()
{
   ...
   assert!(supports_tls("abba[mnop]qrst"), "abba[mnop]qrst");
   assert!(!supports_tls("abcd[bddb]xyyx"), "abcd[bddb]xyyx");
   assert!(!supports_tls("aaaa[qwer]tyui"), "aaaa[qwer]tyui");
   assert!(supports_tls("ioxxoj[asdfgh]zxcvbn"), "ioxxoj[asdfgh]zxcvbn");

   assert!(supports_ssl("aba[bab]xyz"), "aba[bab]xyz");
   assert!(!supports_ssl("xyx[xyx]xyx"), "xyx[xyx]xyx");
   assert!(supports_ssl("aaa[kek]eke"), "aaa[kek]eke");
   assert!(supports_ssl("zazbz[bzb]cdb"), "zazbz[bzb]cdb");
}
...
impl<'a> Iterator for Decompressor<'a>
{
   ...
   fn next(&mut self) -> Option<&'a str>
   {
      ...
      let (len, count) = parse_marker(&self.txt[self.posn + next_marker_pos..self.posn +
                                                                             next_marker_pos +
                                                                             marker_len])
         .unwrap();
      self.ct = count;
...
fn test_bots()
{
   ...
   eval_bots(&mut bots, &mut outputs, &mut comparisons);
   println!("{:?}", outputs);
   println!("{:?}", comparisons);
...
impl FloorPlan
{
   ...
   {
      self.po[0] >= 4 && self.po[1] >= 4 && self.th[0] >= 4 && self.th[1] >= 4 &&
      self.pr[0] >= 4 && self.pr[1] >= 4 && self.ru[0] >= 4 && self.ru[1] >= 4 &&
      self.co[0] >= 4 && self.co[1] >= 4 &&
      self.el[0] >= 4 && self.el[1] >= 4 && self.di[0] >= 4 && self.di[1] >= 4
   }
...
fn asb_opt(src: &[day12::ASB]) -> Vec<ASB>
{
   ...
   for w in src.windows(3)
   {
      ...
      match w
      {
         &[day12::ASB::Inc(r1), day12::ASB::Dec(r2), day12::ASB::JNZ(day12::Val::Reg(r3), -2)]
            if r2 == r3 =>
         {
            v.push(ASB::NoOp);
            v.push(ASB::NoOp);
         ...
         &[day12::ASB::Dec(r1), day12::ASB::Dec(r2), day12::ASB::JNZ(day12::Val::Reg(r3), -2)]
            if r1 == r3 =>
         {
            v.push(ASB::NoOp);
            v.push(ASB::NoOp);
         ...
         &[day12::ASB::Dec(r2), day12::ASB::Inc(r1), day12::ASB::JNZ(day12::Val::Reg(r3), -2)]
            if r2 == r3 =>
         {
            v.push(ASB::NoOp);
            v.push(ASB::NoOp);
         ...
         &[day12::ASB::Dec(r2), day12::ASB::Dec(r1), day12::ASB::JNZ(day12::Val::Reg(r3), -2)]
            if r1 == r3 =>
         {
            v.push(ASB::NoOp);
            v.push(ASB::NoOp);
...
fn test_asb()
{
   ...
   assert!(1 ==
           asb_step(&[day12::ASB::JNZ(day12::Val::Lit(1), -1); 3][..],
                    &mut [0, 0, 0, 0],
                    2));
   asb_step(&[day12::ASB::Cpy(day12::Val::Reg(day12::Reg::A), day12::Reg::B)][..],
            &mut regs,
            0);
...
fn eval_asb_o(code: &[ASB], regs: &mut [i64; 4])
{
   let mut ip = 0;
   while ip < code.len()
   {
      match code[ip]
      {
         ASB::MovL(val, reg) => regs[reg] = val,
         ASB::MovR(r1, r2) => regs[r1] = regs[r2],
         ASB::Jmp(x) if x > 0 =>
         {
            ip += x as usize;
            continue;
         ...
         ASB::AddL(r, x) => regs[r] += x,
         ASB::SubL(r, x) => regs[r] -= x,
         _ => (), // NoOp, Jmp(0)
      }
   }
}
